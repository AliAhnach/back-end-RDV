...
def _build_db_uri() -> str:
    # Priorité 1 : DATABASE_URL (variable universelle)
    url = os.environ.get("DATABASE_URL", "").strip()
    if url:
        if url.startswith("mysql://"):
            url = "mysql+pymysql://" + url[len("mysql://"):]
        log.info("[DB] Source     : DATABASE_URL — %s", _mask(url))
        return url
    ...
    if url:
        if url.startswith("mysql://"):
            url = "mysql+pymysql://" + url[len("mysql://"):]
        log.info("[DB] Source     : MYSQL_URL — %s", _mask(url))
        return url

    # Priorité 3 : variables individuelles MYSQL*
    ...
    if host and user and database:
        ...
        log.info("[DB] Source     : MYSQL* vars — host=%s db=%s", host, database)
        return url

    # Priorité 4 : LOCAL_DATABASE_URL (développement local uniquement)
    url = os.environ.get("LOCAL_DATABASE_URL", "").strip()
    if url:
        log.info("[DB] Source     : LOCAL_DATABASE_URL — %s", _mask(url))
        return url

    log.error("[DB] Aucune variable de base de données définie.")
    return ""
